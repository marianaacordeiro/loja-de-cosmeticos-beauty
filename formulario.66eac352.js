document.addEventListener("DOMContentLoaded", ()=>{
    ...
    if (datepickerElems.length) M.Datepicker.init(datepickerElems, {
        format: "dd/mm/yyyy",
        i18n: {
            months: [
                "Janeiro",
    ...
    if (modalElems.length) M.Modal.init(modalElems);
    // Máscara para Telefone //
    const telefoneInput = document.getElementById("telefone");
    if (telefoneInput) telefoneInput.addEventListener("input", ()=>{
        telefoneInput.value = telefoneInput.value.replace(/\D/g, "").replace(/(\d{2})(\d)/, "($1) $2").replace(/(\d{5})(\d)/, "$1-$2");
    });
    // Máscara para CPF //
    ...
    document.querySelectorAll(".custom-tab").forEach((tab)=>{
        tab.addEventListener("click", ()=>{
            document.querySelectorAll(".custom-tab, .tabs-content").forEach((el)=>el.classList.remove("active"));
    ...
    if (formCadastro) formCadastro.addEventListener("submit", (e)=>{
        ...
        if (localStorage.getItem(email)) {
            alert("Este e-mail j\xe1 est\xe1 cadastrado.");
            return;
        }
        const usuario = {
    ...
    if (formLogin) formLogin.addEventListener("submit", (e)=>{
        ...
        if (usuario && usuario.senha === senha) window.location.href = "index.html";
        else {
            const modalInstance = M.Modal.getInstance(document.getElementById("modal-usuario-nao-cadastrado"));
    ...
    if (senhaInput) {
        senhaInput.addEventListener("invalid", (event)=>{
            event.target.setCustomValidity("Digite uma senha com no m\xednimo 6 caracteres, incluindo n\xfameros e letras.");
        });
...
//# sourceMappingURL=formulario.66eac352.js.map
